...
namespace DASPi{
	...
	template<size_t n>
	ApertureClient<n>::ApertureClient(const in_addr_t clntAddr, const int port, const in_addr_t srvAddr)
	    :sf_{sf_t()},
	    sfdp_{sfdp_t(sf_)},
	    udpClnt_{UDPClnt(clntAddr, port, srvAddr)}
	{
	    log_verbose("[ApertureClient]");
	    ...
	    log_verbose("[ApertureClient - End]");
	}
	
	template<size_t n>
	ApertureClient<n>::~ApertureClient(){
	     log_verbose("~ApertureClient");
	}
	
	template<size_t n>
	std::string ApertureClient<n>::inAddrTToString(in_addr_t ip) {
	...
	///*
	 ...
	 //*/
	//bool ApertureClient::RequestConnection(){
	    //udpClnt_.TransmissionRequest();
	    //return true;
	//}        
	     
	template<size_t n>   
	bool ApertureClient<n>::ReceiveApertureCapture() {
	...
	template<size_t n>
	bool ApertureClient<n>::BufferToFile() {
	    log_verbose("[ApertureClient::BufferToFile]");
	       
	    for(size_t i=0; i<n_+1; i++){
			log_verbose("Writing file:" + std::to_string(i)) ;
	        if (!files_[i]->write( reinterpret_cast<const char*>(buffer_[i].data()),sf_.sf_t::GlobalLinearShapeFunction_t::size()*sizeof(uint16_t))) {
	...
	template<size_t n>
	int ApertureClient<n>::GetPort(){
	#ifdef VERBATIUM_COUT
	    std::cout << "GetPort" << std::endl;
	#endif 
	    return udpClnt_.port_;
	}
	
	template<size_t n>
	std::vector<uint16_t> ApertureClient<n>::GenerateStripedBayerBGGRImage(int width, int height, int stripeWidth) {
	...
	//template<size_t n>
	//inline void ApertureClient<n>::ApplyWhiteBalanceToMosaic_BGGR(
	...
	//template<size_t n>
	//auto ApertureClient<n>::BrightenImage(std::vector<uint8_t> &&input){
	...
	template<size_t n>
	void ApertureClient<n>::BrightenImageInplace(std::span<uint16_t> buf, size_t shift)
	{
	    log_verbose("[ApertureClient::BrightenImageInplace]");
	    if (buf.empty()) return;
	
	    const size_t threads = std::max<size_t>(1, processingThreads_);
	...
	template<size_t n>
	void ApertureClient<n>::BrightenImageChunked2_NEON(uint16_t *buffer, size_t start, size_t end, size_t shift) {
